...
- **Detection:** `detection/detect.py` — finds the 3 seeded bottlenecks in `event_log.parquet`:
  ...
  |----|------|--------|--------------------------|
  | BN001 | delay | Booking Confirmation gap ≥ 7 days | P=0.88 / R=0.97 |
  | BN002 | repetition | a Payment Re-entry step exists | P=1.00 / R=1.00 |
  | BN003 | rework | a Quote Revision step exists | P=1.00 / R=1.00 |
...
- **Domain mismatch** — dashboard chrome is Foyle education (host families/CVs/invoices); the live
  data is a generic booking workflow (Enquiry/Quote/Booking Confirmation/Payment). For pixel-parity
  with the prototype content, regenerate synthetic data into Foyle's actual domain.

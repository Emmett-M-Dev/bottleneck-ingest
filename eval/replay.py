...
log:
    ...
    learning compounds — a fix approved at tick t sits in `sme_resolutions`
        (source="learned") from tick t+1 on, so the learned-hit rate and
        retrieval rank of learned fixes climb as the corpus grows

HONESTY NOTE (write-up must repeat it): the tick-by-tick approver is an
ORACLE, not a human — it approves a suggested fix when the majority of the
cases behind it are genuinely affected per ground truth. It stands in for
the domain expert so nine weeks can replay in minutes; the real Gate 2 with
a real human is the dashboard demo. Its decisions are logged to
outputs/replay_decisions_<profile>.jsonl, never to the dashboard's
decisions.jsonl.

Learned entries live in their own file (outputs/replay_learned_<profile>.json,
ids RES-RPL-*) so the dashboard's real learned resolutions are never touched.
...
def _learn_approved(profile: str, decision: dict) -> dict | None:
    """append_learned, but into the replay's own file with RES-RPL-* ids so
    dashboard-learned entries (RES-LRN-*) are never shadowed in chroma."""
...
def run_tick(profile: str, tick_meta: dict, approved: ApprovedMapping, *,
             llm: bool, decisions_out: list[dict]) -> dict:
    ...
    approved_n = rejected_n = learned_new = 0
    for bn in findings:
        # Retrieval happens BEFORE this tick's approvals are learned, so tick
        # t can only ever see fixes approved at ticks < t.
        ...
        if approve:
            approved_n += 1
            if _learn_approved(profile, decision):
                learned_new += 1
        else:
            rejected_n += 1
...
def main() -> None:
    ...
    replay_decisions_path(args.profile).write_text(
        ...
        encoding="utf-8")
    print(f"-> {log.relative_to(config.ROOT)}  "
          f"({len(records) - 1} ticks, {len(decisions)} gate decisions)")
